...
SELECT * FROM EMP WHERE ENAM='SCOTT'; -- 1행
SELECT * FROM DEPT; -- 4행
-- CROSS JOIN
SELECT *
    FROM EMP, DEPT
    WHERE ENAME='SCOTT'; -- 1(EMP갯수)*4(DEPF갯수) => 4행
...
SELECT EMPNO NO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.DEPTNO, DNAME, LOC -- (3)
    ...
    -- EX2. 급여가 2000이상인 직원의 이름, JOB, 급여, 부서명, 근무지
    SELECT ENAME, JOB, SAL, DNAME, LOC
        FROM EMP E, DEPT D WHERE E.DEPTNO=D.DEPTNO AND SAL>=2000;
    -- EX3. 근무지가 CHICAGO인 직원의 이름, JOB, 급여, 부서번호 출력
    SELECT ENAME, JOB, SAL, DEPTNO FROM EMP E, DEPF D
...
SELECT WORKER.EMPNO, WORKER.ENAME, WORKER.MGR, MANAGER.EMPNO, MANAGER.ENAME
    ...
    SELECT E1.ENAME, E1.DEPTNO, E2.ENAME, E2.DEPTNO
        ...
        AND ENAME^='SCOTT'; -- 서브쿼리 이용
    -- SCOTT과 동일한 근무지에서 근무하는 사원의 이름을 출력하시오(2단계 최종문제) 
    -- 6장 수업후 subquery 사용 추천
------------------
--★ 4. OUTER JOIN ★ SELF JOIN, EQUL JOIN시 조건이 만족하지 않는 형까지 나타나게하는 JOIN
------------------
...
SELECT ENAME,
(SELECT ENAME 
FROM EMP M WHERE M.EMPNO = E.MGR) AS MANAGER_NAME
FROM EMP E;
--2. 이름, 급여, 업무, 직속상사명
